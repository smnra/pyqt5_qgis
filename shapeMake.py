...
class CsvData():
    ...
    def newMapFile(self,i):
        ...
        # 生成 字段列表
        dataSource = newMap.newFile('pylgon_aaa_' + str(i) +'.shp')
        # 创建shape文件
        ...
        for i,csvLine in enumerate(datas):
            if csvLine:
                ...
                if i%1000000==0:
                    ...
                    # (("index", (4, 254)), ("name", (4, 254)), ("lon", 2), ("lat", 2))
                    # 生成 字段列表
                    dataSource = newMap.newFile('pylgon_bbb_' + str(i) + '.shp')
                    # 创建shape文件
                    newLayer = newMap.createLayer(dataSource, fieldList)
                    # 创建 图层Layer 对象
...
if __name__=="__main__":
    csvData = CsvData(r'./input/geo.csv')
    # 初始化类
